...
scenario = st.sidebar.selectbox(
    "Scenario",
    ["Basic", "Wind", "Wind + Deposition", "Comparison of scenarios"]
)
...
if st.button("Run simulation"):
    ...
    if scenario != "Comparison of scenarios":
        ...
        with left:
            st.subheader("Particle cloud")
            cloud_box = st.empty()
            st.subheader("Number of airborne particles over time")
            count_box = st.empty()
        ...
        for t in range(len(X)):
            ...
            count_box.line_chart(df["count"].iloc[:t+1])

            mean_box.line_chart(df[["mean_x","mean_y"]].iloc[:t+1], height=120)
            var_box.line_chart(df[["var_x","var_y"]].iloc[:t+1], height=120)
            std_box.line_chart(df[["std_x","std_y"]].iloc[:t+1], height=120)
    ...
    # =========================
        ...
        st.subheader("Comparison of the number of airborne particles over time")
        ...
        st.subheader("Comparison of final moments")

        final_df = pd.DataFrame({
            "Basic": d1.iloc[-1],
            "Wind": d2.iloc[-1],
            "Wind + Deposition": d3.iloc[-1]
        }).T
