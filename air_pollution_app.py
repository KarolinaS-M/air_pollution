...
if st.button("Run simulation"):
    ...
    # Layout: top (cloud + mean), bottom (variance + std)
    ...
    for t in range(len(X)):
        ...
        mean_box.line_chart(df[["mean_x", "mean_y"]].iloc[:t+1], height=130)
        var_box.line_chart(df[["var_x", "var_y"]].iloc[:t+1], height=130)
        std_box.line_chart(df[["std_x", "std_y"]].iloc[:t+1], height=130)
